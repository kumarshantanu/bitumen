...
import springer.util.MapBuilder;
...
public class DITest {
	...
	public void testConstantlyReturnsConstant() {
		...
		final IComponentSource<String, Bean> sf = DI.constantly(strConstant, Bean.class);
		assertEquals(strConstant, sf.get(new HashMap<DITest.Bean, IComponentSource<?,Bean>>()));
		final int intConstant = 100;
		final IComponentSource<Integer, Bean> inf = DI.constantly(intConstant, Bean.class);
		assertEquals((Integer) intConstant, inf.get(new HashMap<DITest.Bean, IComponentSource<?,Bean>>()));
	}

	@Test
	public void testSingletonProducesExactlyOneInstance() {
		final AtomicInteger counter = new AtomicInteger(0);
		final IComponentSource<?,Bean> factory = new IComponentSource<String,Bean>() {
			@Override
			public String get(Map<Bean, IComponentSource<?, Bean>> dependencies) {
				counter.incrementAndGet();
				return "hello";
			}
		};
		final IComponentSource<?,Bean> singleton = DI.singleton(factory);
		// before invocation
		assertEquals(0, counter.get());
		// first invocation - counter should increment
		assertEquals("hello", singleton.get(new HashMap<Bean, IComponentSource<?,Bean>>()));
	...
	public void testConstruct() {
		...
		assertEquals("Happy scenario - single constructor argument", "Hello", hello.get(Collections.EMPTY_MAP));
		...
		assertEquals("Happy scenario - multiple constructor args", "ello", ello.get(Collections.EMPTY_MAP));
		// unhappy scenario
		...
		try {
			assertEquals("Unhappy scenario", "", nostr.get(Collections.EMPTY_MAP));
	...
	public void testSourceOf() {
		...
		// unhappy scenario
		try {
			assertEquals("Unhappy scenario", -1, source.get(Collections.EMPTY_MAP));
			fail("Expected to throw " + IllegalArgumentException.class + " but it did not");
		} catch(IllegalArgumentException e) {
			// do nothing
		}
